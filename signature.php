...
<html>
	<head>
		<meta charset="UTF-8">
		<title>Тест: Электронная подпись</title>
		...
		<script>
			...
			function submit(chosenQuestions) {			// отправляет ответы на сервер, chosenQuestions – массив с номерами выбранных вопросов
				if (document.getElementById('name').value == "") {					// если имя не введено
					...
					questionsInTest = 11;			// количество вопросов в этом тесте
					testName = "signature";	// имя теста
					...
					for (var i = 0; i < questionsInTest; i++) {
						if (i == 3 || i == 6 || i == 10) {			// для вопроса с radiobuttons
							test[i + 3] = $('[name="question' + i + '"]:checked').val();				// номер выбранного ответа
	...
	<body>
		...
		<div id="page">
			<div id="testsbox">
				<h2><center>Тест: Электронная подпись</center></h2>
        <center><p id="introduce" style="margin-bottom: 23px;"><b>Представьтесь, пожалуйста: </b><input type="text" id="name" style="width: 370px;" maxlength="41"></p></center>
				<center><?php echo file_get_contents("./images/hint2.svg"); // загружаем подсказку ?></center>
				<?php 
//				показываем ошибки:
...
//				error_reporting(E_ALL);
					...
					// -- нулевой вопрос -- //
					$myFile = "./tests/signature/questions_0.txt";
          ...
          array_push($chosenQuestions, $question);	// записываем номер выбранного вопроса
					...
					// -- первый вопрос -- //
					$myFile = "./tests/signature/questions_1.txt";
					$questions = file($myFile);			// загружаем файл в массив
					$lines = count($questions) - 1;	// подсчитываем количество возможных вопросов
					$question = rand(0, $lines);		// выбираем случайный из них
          ...
          array_push($chosenQuestions, $question);	// записываем номер выбранного вопроса
					...
					// -- второй вопрос -- //
					$myFile = "./tests/signature/questions_2.txt";
					$questions = file($myFile);			// загружаем файл в массив
					$lines = count($questions) - 1;	// подсчитываем количество возможных вопросов
					$question = rand(0, $lines);		// выбираем случайный из них
          ...
          array_push($chosenQuestions, $question);	// записываем номер выбранного вопроса
					...
					$myFile = "./tests/signature/questions_3.txt";
					$questions = file($myFile);			// загружаем файл в массив
					$options = explode(",", $questions[$question]);		// сам вопрос и варианты ответов (5 штук) для данного вопроса
					echo '<p id="3" style="margin-bottom: 6px;">' . $options[0] . '</p>';	// выводим его
					echo '<span class="mono" style="margin-top: 0px;"><input type="radio" name="question3" value="1">' . $options[1] . '; ';
					echo '<input type="radio" name="question3" value="2">' . $options[2] . '; ';
					echo '<input type="radio" name="question3" value="3">' . $options[3] . '; ';
					echo '<input type="radio" name="question3" value="4">' . $options[4] . '; ';
					echo '<input type="radio" name="question3" value="5">' . $options[5] . '</span><br>';
          array_push($chosenQuestions, $question);	// записываем номер выбранного вопроса

					// -- четвёртый вопрос -- //
					// выбираем вопрос с тем же номером, что и у предыдущего
					$myFile = "./tests/signature/questions_4.txt";
					$questions = file($myFile);			// загружаем файл в массив
          ...
          array_push($chosenQuestions, $question);	// записываем номер выбранного вопроса
					...
					// -- пятый вопрос -- //
					// выбираем вопрос с тем же номером, что и у предыдущего
					$myFile = "./tests/signature/questions_5.txt";
					$questions = file($myFile);			// загружаем файл в массив
          ...
          array_push($chosenQuestions, $question);	// записываем номер выбранного вопроса
					...
					$myFile = "./tests/signature/questions_6.txt";
					$questions = file($myFile);			// загружаем файл в массив
					$lines = count($questions) - 1;	// подсчитываем количество возможных вопросов
					$question = rand(0, $lines);		// выбираем случайный из них
					$options = explode(",", $questions[$question]);		// сам вопрос и варианты ответов (5 штук) для данного вопроса
					echo '<p id="6" style="margin-bottom: 6px;">' . $options[0] . '</p>';	// выводим его
					echo '<span class="mono" style="margin-top: 0px;"><input type="radio" name="question6" value="1">' . $options[1] . '; ';
					echo '<input type="radio" name="question6" value="2">' . $options[2] . '; ';
					echo '<input type="radio" name="question6" value="3">' . $options[3] . '; ';
					echo '<input type="radio" name="question6" value="4">' . $options[4] . '; ';
					echo '<input type="radio" name="question6" value="5">' . $options[5] . '</span><br>';
          array_push($chosenQuestions, $question);	// записываем номер выбранного вопроса

					// -- седьмой вопрос -- //
					// выбираем вопрос с тем же номером, что и у предыдущего
					$myFile = "./tests/signature/questions_7.txt";
					$questions = file($myFile);			// загружаем файл в массив
          ...
          array_push($chosenQuestions, $question);	// записываем номер выбранного вопроса
					...
					// -- восьмой вопрос -- //
					// выбираем вопрос с тем же номером, что и у предыдущего
					$myFile = "./tests/signature/questions_8.txt";
					$questions = file($myFile);			// загружаем файл в массив
          ...
          array_push($chosenQuestions, $question);	// записываем номер выбранного вопроса
					...
					// -- девятый вопрос -- //
					// выбираем вопрос с тем же номером, что и у предыдущего
					$myFile = "./tests/signature/questions_9.txt";
					$questions = file($myFile);			// загружаем файл в массив
					echo '<p id="9">' . $questions[$question] . '</p>';	// выводим его
					echo '<p>Ответ: <input type="text" class="answer" id="answer9" style="width: 370px;" maxlength="47"></p><br>';	// и поле для ответа
          array_push($chosenQuestions, $question);	// записываем номер выбранного вопроса

					// -- десятый вопрос -- //
					// выбираем вопрос с тем же номером, что и у предыдущего
					$myFile = "./tests/signature/questions_10.txt";
					$questions = file($myFile);			// загружаем файл в массив
					$options = explode(",", $questions[$question]);		// сам вопрос и варианты ответов (два) для данного вопроса
					echo '<p id="10" style="margin-bottom: 6px;">' . $options[0] . '</p>';	// выводим его
					echo '<span class="mono" style="margin-top: 0px;"><input type="radio" name="question10" value="1">' . $options[1] . '<br>';
					echo '<input type="radio" name="question10" value="2">' . $options[2] . '</span><br>';
          array_push($chosenQuestions, $question);	// записываем номер выбранного вопроса
